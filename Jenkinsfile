pipeline {
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                checkout scmGit(branches: [[name: '**']], extensions: [], userRemoteConfigs: [[credentialsId: 'my_gitlab', url: 'http://172.31.33.137/root/weather_app_repo.git']])
            }
            post {
                failure {
                    script {
            ...
            steps {
                ...
                sshagent(credentials: ['weather_app_instance']) {
                   // clean the dir for new work
                    ...
                    //  the script clean the images and then run the docker compose
